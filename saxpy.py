...
def saxpy(target,dtype_op1,dtype_op2,dtype_store, length=1,unroll=0,vectorize=0, lane=4):
    ...
    if length != 1:
        ...
        #C = tvm.topi.cast(te.compute((length,), lambda i: te.sum(A[i] * B[i], axis=p), name='C'), dtype_store)
        if (dtype_op1 == dtype_op2 and dtype_op1 == dtype_store):
            Z = te.compute((length,), lambda i: alpha * X[i] + Y[i], name='Z') #, dtype=dtype_store)
        else:
            Z = tvm.topi.cast(te.compute((length,), lambda i: alpha * X[i] + Y[i], name='Z') , dtype=dtype_store)
    
    else:
        X = te.var("X", dtype=dtype_op1)
        Y = te.var("Y", dtype=dtype_op2)
        alpha= te.var("alpha", dtype=dtype_op1)
        #C = tvm.topi.cast(te.compute((length,), lambda i: te.sum(A * B, axis=0), name='C'), dtype_store)
        #Z = te.compute((1,), lambda i: alpha * X + Y, name='Z') #, dtype=dtype_store)
        #C = te.compute((length,), lambda i: te.sum(A.astype(dtype_store) * B.astype(dtype_store), axis=p), name='C')
    ...
    if vectorize == 1:
        ...
        s[Z].vectorize(xi)
    n = 'saxpy_length_{}_op1_{}_op2_{}_opstore_{}'.format(length,dtype_op1,dtype_op2,dtype_store)
    """
...
if __name__ == "__main__":
    ...
    size=1000
    lane = 8
    func = saxpy(target,dtype_op1, dtype_op2,dtype_store,size,0,1,lane)
